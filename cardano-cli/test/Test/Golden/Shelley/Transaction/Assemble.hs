...
module Test.Golden.Shelley.Transaction.Assemble
  ...
  , golden_shelleyTransactionAssembleWitness_SingleSigningKey
  ) where
...
golden_shelleyTransactionAssembleWitness_SingleSigningKey :: Property
golden_shelleyTransactionAssembleWitness_SingleSigningKey = propertyOnce $ moduleWorkspace "tmp" $ \tempDir -> do
  -- Test assembling an any multisig script
