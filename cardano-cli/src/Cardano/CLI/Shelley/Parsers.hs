...
pScriptOrSigningKey =
  Opt.flag' ()
    (  Opt.long "script-based-witness"
    <> Opt.help "Use a script to construct the tx witness"
    ) *> pScript'
  <|>
    Opt.flag' ()
    (  Opt.long "signing-key-based-witness"
    <> Opt.help "Use a signingkey to construct the tx witness"
...
pScript' =
  ScriptFileForWitness <$>
    Opt.strOption
      (  Opt.long "script-file"
      <> Opt.metavar "FILE"
      <> Opt.help "Filepath of the witness multisig script."
      <> Opt.completer (Opt.bashCompleter "file")
      )
...
pMultiSigScriptObject = pAny <|> pAll <|> pAtLeast
 where
   ...
   pAll = Opt.flag' () (  Opt.long "all"
                       <> Opt.help "Build an any all signature script.")
   ...
   pRequired = Opt.option Opt.auto
                 (  Opt.long "required"
                 <> Opt.metavar "INT"
                 <> Opt.help "The minimum required number of signatures."
                 )
